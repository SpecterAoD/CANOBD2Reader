...
namespace {
  ...
  Config::VoltageMeasurement readVinVoltage() {
    Config::VoltageMeasurement measurement;
    measurement.adcRaw = analogRead(Config::Sender::VoltageDividerPin);
    const float adcVoltage = measurement.adcRaw * (3.3f / 4095.0f);
    measurement.voltage = adcVoltage * Config::Sender::VoltageCalcFactor;
    lastVoltage = measurement.voltage;
    return measurement;
  }

  void enterDeepSleep() {
    esp_sleep_enable_timer_wakeup(static_cast<uint64_t>(Config::Sender::SleepPeriodSec) * 1000000ULL);
...
namespace SenderPower {
  void reduceHeat() {
    setCpuFrequencyMhz(Config::Sender::CpuFrequency);
  }
  ...
  void updateCarStatus() {
    ...
    if (carStatus == CarIsRunning &&
        millis() - lastCarRunningTime < Config::Sender::StartStopDelayMs &&
        measurement.voltage > (12.0f + Config::Sender::VoltageChangeThreshold)) {
      return;
    }
  ...
  void handleSleep() {
    ...
    if (millis() - sleepTriggerTime >= Config::Sender::StartStopDelayMs) {
      enterDeepSleep();
    }
  }
